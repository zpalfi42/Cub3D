...
/*   Updated: 2022/10/31 13:11:53 by zpalfi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_new_left_str(char *left_str)
{
	...
	str = (char *)malloc(sizeof(char) * (ft_strlen_2(left_str) - i + 1));
	if (!str)
		return (NULL);
	i++;
	j = 0;
...
static char	*ft_read_to_left_str(int fd, char *left_str)
{
	...
	while (!ft_strchr_2(left_str, '\n') && rd_bytes != 0)
	{
		...
		buff[rd_bytes] = '\0';
		left_str = ft_strjoin_2(left_str, buff);
	}
	free(buff);
	return (left_str);
}
